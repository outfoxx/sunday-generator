...
val Annotations.fragmentName: String? get() = this.fragmentName().getOrNull()
...
val Shape.location: String?
  get() =
    _internal()
      ...
      ?: annotations.location().orElse(null)

val Shape.uniqueId: String
  get() =
    location
      ?.let { location ->
        annotations.fragmentName
          ?.let { fragmentName ->
            "$location#$fragmentName/$name"
          }
          ?: "$location/$name"
      }
      ?: id
