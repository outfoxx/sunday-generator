...
class GeneratedApiIrExporter
  ...
  constructor(
    ...
    private val openApiDocumentLoader: OpenApiDocumentLoader? = null,
  ) {
    ...
    private fun exportFragment(
      source: GeneratedApiIrSource,
      loader: OpenApiDocumentLoader,
    ): GeneratedApiFragment {
      val sourceKind =
        if (source.kind ==
          GeneratedApiIrSourceKind.AUTO
        ) {
          detectSourceKind(source.uri, loader)
        } else {
          source.kind
        }
      ...
      if (sourceKind == GeneratedApiIrSourceKind.OPENAPI) {
        return openApiMapper.convertFragment(source.uri, loader)
      }
      val result =
        when (sourceKind) {
    ...
    private fun detectSourceKind(
      sourceUri: URI,
      loader: OpenApiDocumentLoader,
    ): GeneratedApiIrSourceKind {
      val text = loader.load(sourceUri).bytes.toString(Charsets.UTF_8)
