...
class ProjectView(SundayModel):
    ...
    @field_validator("release_date", "home_page", "avatar", "display_name", "tags", mode="after")
    @classmethod
    def _validate_non_nullable_optional(cls, value: object, info: ValidationInfo) -> object:
        if value is not None:
            return value
        if info.field_name == "release_date":
            raise ValueError("Property 'releaseDate' is not nullable")
        if info.field_name == "home_page":
            raise ValueError("Property 'homePage' is not nullable")
        if info.field_name == "avatar":
