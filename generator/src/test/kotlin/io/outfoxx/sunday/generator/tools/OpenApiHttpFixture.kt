...
/** Local HTTP fixture shared by reference resolution and compiler-backed output tests. */
class OpenApiHttpFixture : AutoCloseable {
  ...
  /** Handlers for individual resource paths. */
  ...
  init {
    ...
    server.createContext("/") { exchange ->
      exchange.use {
        ...
        handlers[it.requestURI.path]?.invoke(it) ?: it.sendResponseHeaders(404, -1)
      }
    }
    server.start()
  }
  ...
  fun respond(
    ...
    handlers[path] = { exchange ->
      ...
      exchange.sendResponseHeaders(status, if (status == 304) -1 else bytes.size.toLong())
      if (status != 304) exchange.responseBody.write(bytes)
    }
  }
  ...
  fun export(directory: Path): GeneratedApi {
    ...
    respond(
      ...
      """
      openapi: 3.1.0
      info: {title: Shared resources, version: 1.0.0}
      paths: {}
      components:
        schemas:
          Anything: true
          ...
          MappedPet:
            ...
            discriminator:
              propertyName: kind
              mapping: {kitty: '?schema=cat', hound: mapped-dog}
          Cat:
            allOf:
