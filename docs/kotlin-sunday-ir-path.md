...
Broker service facades are generated by default for supported AMQP operations. Use `-no-broker` on the CLI or
`generateBrokerServices.set(false)` in the Gradle plugin to omit broker facades while retaining message models and
protocol binding metadata. Other runtimes default broker generation off and reject `-broker` until they support it.
