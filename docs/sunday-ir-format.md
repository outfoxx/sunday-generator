...
Reference wrappers whose effective contract is unchanged reuse the canonical type before inline model allocation, including redundant `nullable` annotations and single-reference `allOf` wrappers. This uses dialect-aware conjunction analysis; it does not discard nullable assertions generally. Retained aliases do not introduce discriminator subclasses.
...
The same annotation merging applies when `allOf` operands repeat a recursive property or another supported schema-valued keyword. Changing documentation on matching direct references or matching nested wrappers preserves their canonical type without expanding the recursive target. Supplied annotations override earlier ones, omitted annotations remain inherited, and parent declarations stay unchanged. Actual refinements and unproven recursive intersections continue through the existing constraint handling and source-located diagnostics.
...
Before each HTTP request, including redirects, the default loader resolves the destination and rejects any non-public address in the result. This includes loopback, private, link-local, shared/CGNAT, multicast, reserved, and non-global address ranges in IPv4 and IPv6, including embedded IPv4 addresses. Empty or failed DNS resolution fails retrieval, and waiting for DNS is bounded by the request timeout. Configured proxies are also rejected by default because they can resolve destinations independently. Trusted specifications can opt into private destinations and configured proxies with `OpenApiReferenceOptions.allowPrivateNetwork`, the CLI's `--openapi-allow-private-network`, or Gradle's `openApiAllowPrivateNetwork`. This is a broad network opt-in; the existing URI, authentication, redirect, and size restrictions still apply. Offline and captured-document loading make no DNS or HTTP requests and do not require this opt-in.

These checks are DNS preflight validation, not connection pinning. Java 21's HTTP client resolves independently when connecting, so DNS changes between validation and connection remain a limitation. Builds that process untrusted specifications should also enforce network egress restrictions. Injected document loaders own their transport policy.

Remote documents cannot reference local files, even when private-network access is enabled or the local document is already loaded, cached, or captured. This boundary uses effective retrieval origins after redirects; schema `$id` values do not grant local-file access. Registered embedded resources with `file:` identifiers still work when they require no local-file access. Local specifications retain local-to-local and local-to-remote reference support. Violations identify the referring `$ref` or discriminator mapping's source location without reading the local target.

CLI export and generation commands accept `--openapi-reference-cache-dir` (default: `~/.cache/sunday-generator/openapi`), `--openapi-offline`, and `--openapi-allow-private-network` (disabled by default). For example:
...
sunday ir --openapi-allow-private-network -out api.ir.yaml trusted-internal-api.yaml
```

The library exposes `OpenApiReferenceOptions`, `OpenApiDocumentLoader`, and `OpenApiReferenceResolver`. The resolver returns normalized content and captured source documents. `OpenApiDocumentSnapshot` persists those documents for subsequent conversions that must use exactly the same inputs. Existing converter and exporter calls use the default loader; injected loaders can provide captured documents without network access. See the [implementation guide](openapi-reference-implementation.md) for shared schema traversal, conversion-local analysis, and test support.

Gradle's `openApiReferenceCacheDirectory` defaults to `caches/sunday/openapi` beneath Gradle User Home. Gradle `--offline` also applies to OpenAPI retrieval. `openApiAllowPrivateNetwork` defaults to `false` and is a discovery task input. Discovery for generations containing OpenAPI runs on each build, resolves the transitive dependency graph, and captures source content before grouping or generation. The generation task remains cacheable: unchanged content leaves it up to date, and changes to referenced content invalidate it. Capture manifests use relative local paths when the base and target share a filesystem root. Files on different Windows drives, UNC shares, or filesystems retain absolute file URIs and keep those locations when the project moves. Both forms use the existing version-one manifest, excluding HTTP validators and retrieval timestamps. Generations containing only RAML or AsyncAPI retain their existing discovery caching. Empty source sets skip discovery and generation with `NO-SOURCE`. Removing the last specification cleans previous outputs (which Gradle reports as successful work); subsequent empty builds skip, and adding a source enables generation again.

```kotlin
sundayGenerations {
  create("client") {
    openApiReferenceCacheDirectory.set(layout.projectDirectory.dir(".sunday-cache"))
    // Enable only when the specifications and their dependencies are trusted.
    openApiAllowPrivateNetwork.set(true)
  }
}
```
...
Each requirement's `permissions` map retains required OAuth/OIDC scopes or role names under the exact scheme name. Schemes within one requirement are conjunctive; separate requirements are alternatives. Empty permission lists are omitted from the map. OpenAPI schemes also preserve `openIdConnectUrl` and `oauthFlows`, keyed by flow name, with `authorizationUrl`, `tokenUrl`, `refreshUrl`, and advertised `scopes`. These fields default to empty/null for existing IR documents. See [OpenAPI security enforcement](openapi-security-enforcement.md) for opt-in server evaluation and credential bindings.

AsyncAPI 2.x requirement maps and 3.x inline/reference security objects normalize to these same named requirements. In 3.x, scheme `scopes` become required permissions, while OAuth flow `availableScopes` become advertised metadata in IR `oauthFlows.scopes`. AsyncAPI 2.x reads advertised metadata from flow `scopes`. Local security references retain component binding names; equivalent inline authentication definitions receive stable `inline_<sha256>` names independent of required permissions. Applicable server and operation requirements are conjoined during conversion. External AsyncAPI security references are rejected explicitly. HTTP API keys preserve their exact header, query, or cookie wire names.
...
### Inherited scalar restrictions

IR v1 model properties may include optional `allowedValues`, a list of permitted scalar wire values. Omitting the field (or setting it to null) means there is no additional restriction; an empty list permits no supplied value. A list can include JSON null, zero, and false as distinct values. Existing IR documents without this metadata remain readable. IR composition includes restrictions in model compatibility checks.

Python validates these restrictions before field coercion, including Python constructor names, wire aliases, and declared defaults. Discriminator literals retain Pydantic's variant selection and validate restrictions after literal validation. An omitted optional field without a default remains omitted; its internal `None` placeholder does not count as a supplied value. Kotlin Sunday patch models validate set payloads while preserving `none` and deletion operations. Patch parameters default to `none()` and never turn schema defaults into implicit updates. Ordinary requiredness/nullability checks do not replace the runtime's patch operation rules.

A child can retain an inherited enum or model declaration type while supplying property entries with stricter constraints. Frontends emit the effective child constraints, and emitters retain the ancestor storage declaration where the language requires it. Enum restrictions compare wire values, so tolerant enums remain tolerant in unconstrained uses and reject excluded values in constrained descendants. Supported scalar defaults apply when fields are omitted; requiredness and nullability remain separate. Inherited inline objects and unions also retain the declaring property's canonical type across aliases and ancestor chains. Swift formatted defaults use validated Foundation UUID, URL, Date, and base64 Data expressions. Timezone-free dates and times use UTC, date-only values use midnight, and time-only values use 1970-01-01. Defaults never replace explicit null or create implicit patch updates. Python inherited lists with `uniqueItems: true` remain lists; validation rejects structurally equal wire values before coercion, including nested objects and arrays, while keeping booleans distinct from numbers.

Generated SDK output can consequently differ from older betas: canonical wrappers no longer create synthetic subtypes, compatible refinements preserve parent APIs and enum types, child decoding enforces its restrictions, and resolved headers, stronger validation metadata, defaults, and collision-free model names remain supported. Retrieval policy, CLI options, reference caches, and snapshots are unchanged.
