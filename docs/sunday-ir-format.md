...
AsyncAPI 2.x requirement maps and 3.x inline/reference security objects normalize to these same named requirements. In 3.x, scheme `scopes` become required permissions, while OAuth flow `scopes` remain advertised metadata. Local security references retain component binding names; equivalent inline authentication definitions receive stable `inline_<sha256>` names independent of required permissions. Applicable server and operation requirements are conjoined during conversion. External AsyncAPI security references are rejected explicitly. HTTP API keys preserve their exact header, query, or cookie wire names.
